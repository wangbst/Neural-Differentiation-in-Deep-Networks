...
# Function to prune neuron based on NDI
def prune_neuron_NDI(model, NDI_values, target_sparsity=):
    ...
    # Collect all neuron and their importance scores
    all_neuron = []
    importance_scores = []
...
for epoch in range(100):  # loop over the dataset
    ...
    if current_sparsity < target_sparsity:
        prune_neuron_NDI(model, NDI_values, target_sparsity=target_sparsity)
